"""
Reference code for LLaMA-3.2 training and inference.
...
def log(name, tensor):
    ...
    flat_w = tensor.flatten()[:10].cpu().tolist()
    rounded_w = [f"{v:.7f}" for v in flat_w]
    print(f"  → {name}, shape={tuple(tensor.shape)}")
    print(f"     range: min={min_w:.7f}, max={max_w:.7f}, values={rounded_w}")
...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ndim = x.ndim
    assert 0 <= 1 < ndim
    assert freqs_cis.shape[:-1] == (x.shape[1], x.shape[-1] // 2)
    return freqs_cis.view(1, x.shape[1], 1, x.shape[-1] // 2, 2)
...
def apply_rotary_emb(
    ...
    def apply(x):
        ...
        left = x1 * cos - x2 * sin
        right = x1 * sin + x2 * cos
        rotated = torch.stack([left, right], dim=-1)
        return rotated.flatten(-2)

    return apply(xq), apply(xk)

def precompute_freqs_cis(
    dim: int, end: int, theta: float = 10000.0, use_scaled: bool = False, device = torch.device("cuda")
):
...
class LLaMA(nn.Module):
    ...
    def from_pretrained_llama3_hf(cls, model_id):
        ...
        assert model_id == "meta-llama/Llama-3.2-1B", "Only the 1B-base model is supported for now"
        ...
        torch.set_default_tensor_type(torch.cuda.FloatTensor)  # much faster loading
        ...
        tokenizer.pad_id = 128004  # this is the pad token id for LLaMA 3.2 base, we need to set this explicitly as our generate func expects it
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type, zero_stage):
        ...
        if zero_stage == 1:
            ...
            print0("using regular Adam")
            optimizer = torch.optim.Adam(optim_groups, lr=learning_rate, betas=(0.9, 0.99))
            # optimizer = torch.optim.Adam(optim_groups, lr=learning_rate, betas=betas, fused=use_fused)
        return optimizer
...
def write_model(model, filename, dtype):
    ...
    header[8] = to_int32(model.config.ffn_dim_multiplier)
    header[9] = model.config.multiple_of
    header[10] = to_int32(model.config.norm_eps)
    header[11] = to_int32(model.config.rope_theta)
...
def write_tokenizer(tokenizer, filename: str):
    ...
    - Header: 256 * int32
        - [0] magic number (e.g. 20240530)
        - [1] version (e.g. 3)
        - [2] total number of tokens
        - [3] EOT token ID
        - [4...] reserved
    - Body:
        - for each token_id in range(n):
            - 1-byte: token length
            - N-byte: token bytes
    """
    n = len(tokenizer)
    ...
    header[3] = eot_token_id if eot_token_id is not None else -1

    with open(filename, "wb") as file:
        file.write(header.numpy().tobytes())
        for token_id in range(n):
            token = tokenizer.decode(token_id)
            byte_content = token.encode("utf-8")
            length = len(byte_content)
            assert length < 256, f"Token length exceeds 255: {length}"
            file.write(struct.pack("<B", length))
            file.write(byte_content)

    print(f"write tokenizer from huggingface: wrote {filename} with {n} tokens.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_bin", type=str, default="/data/shared/InfiniTrain-dev/data/llmc/llama3/tinyshakespeare/tiny_shakespeare_train.bin", help="input .bin to train on")
    ...
    if args.use_hf:
        ...
        write_tokenizer(model.tokenizer, f"llama3_tokenizer.bin")
    ...
    if args.precision_hook:
        ...
        sys.path.insert(0, os.path.join(root, 'tools/precision_check'))
        from pytorch_hook import PrecisionHook
        precision_hook = PrecisionHook(args.precision_hook)
        precision_hook.register(model)
        print0(f"Precision hook enabled, output to: {args.precision_hook}")

    def print_tensor_range_hook(name):
            ...
            if isinstance(input, (tuple, list)):
                for i in range(len(input)):
                    if isinstance(input[i], torch.Tensor):
                        flat = input[i].flatten()[:10].cpu().tolist()
                        rounded = [f"{v:.4f}" for v in flat]
                        print(f"[{name}] input {i}: {rounded}")

            if isinstance(output, torch.Tensor):
                min_val = output.min().item()
                max_val = output.max().item()
                print(f"[{name}] output range: min={min_val:.4f}, max={max_val:.4f}")
                flat = output.flatten()[:10].cpu().tolist()
                rounded = [f"{v:.4f}" for v in flat]
                print(f"[{name}] output 0: {rounded}")

            for name_param, param in module.named_parameters(recurse=False):  
                min_w = param.min().item()
                max_w = param.max().item()
                flat_w = param.flatten()[:10].cpu().tolist()
                rounded_w = [f"{v:.4f}" for v in flat_w]
                print(f"  → Param: {name_param}, shape={tuple(param.shape)}")
                print(f"     range: min={min_w:.4f}, max={max_w:.4f}, values={rounded_w}")

        return hook

    def register_range_hooks(model):
        for name, module in model.named_modules():
            if isinstance(module, torch.nn.Module) and not isinstance(module, torch.nn.Sequential):
                module.register_forward_hook(print_tensor_range_hook(name))

    def print_tensor_grad_range_hook(name):
        def hook(module, grad_input, grad_output):
            ...
            for i, go in enumerate(grad_output):
                if isinstance(go, torch.Tensor):
                    flat = go.flatten()[:10].cpu().tolist()
                    rounded = [f"{v:.6e}" for v in flat]
                    print(f"[{name}] grad_output {i}: {rounded}, range: [{go.min():.6f}, {go.max():.6f}]")

            for i, gi in enumerate(grad_input):
                if isinstance(gi, torch.Tensor):
                    flat = gi.flatten()[:10].cpu().tolist()
                    rounded = [f"{v:.6e}" for v in flat]
                    print(f"[{name}] grad_input {i}: {rounded}, range: [{gi.min():.6f}, {gi.max():.6f}]")
            ...
            for name_param, param in module.named_parameters(recurse=False):
                ...
                rounded_w = [f"{v:.4f}" for v in flat_w]
                print(f"  → Param: {name_param}, shape={tuple(param.shape)}")
                print(f"     range: min={min_w:.6f}, max={max_w:.6f}, values={rounded_w}")

                if param.grad is not None:
                    g = param.grad
                    flat = g.flatten()[:10].cpu().tolist()
                    rounded = [f"{v:.6e}" for v in flat]
                    print(f"  → Param Grad: {name_param}, shape={tuple(g.shape)}")
                    print(f"     range: min={g.min():.6f}, max={g.max():.6f}, values={rounded}")
    ...
    def register_backward_hooks(model):
        for name, module in model.named_modules():
            if isinstance(module, torch.nn.Module) and not isinstance(module, torch.nn.Sequential):
                module.register_full_backward_hook(print_tensor_grad_range_hook(name))

    # register_range_hooks(model)
    ...
    if master_process and args.write_tensors and (not args.inference_only):
        ...
        model_to_size = {"meta-llama/Llama-3.2-1B": "1B"}
        model_size_str = model_to_size[args.model] # e.g. "1B"
        write_model(model, os.path.join(args.output_dir, f"llama3.2_{model_size_str}_fp32.bin"), dtype="float32")
    ...
    for step in range(args.num_iterations + 1):
        ...
        if (args.sample_every > 0 \
            ...
            start_ids = [model.tokenizer.eos_token_id]
            xg = (torch.tensor(start_ids, dtype=torch.long, device=device)[None, ...])
            prompt = "The meaning of life is"
            xg = model.tokenizer.encode(prompt, return_tensors="pt").to(device)
            max_new_tokens = 5
            temperature = 1.0
            top_k = 40
            yg = raw_model.generate(xg, max_new_tokens, temperature=temperature, top_k=top_k)
            print0('---------------')
            print0(model.tokenizer.decode(yg[0].tolist()))
            print0('---------------')
